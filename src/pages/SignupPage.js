...
function App() {
    ...
    // 폼 유효성 검사(Form Validation Check) 관련 state 정의 : 입력 양식에 문제 발생 시 값을 저장할 곳.
    ...
    const SignupAction = async (event) => {
        ...
        try {
            ...
            if (error.response && error.response.data) {
                // 서버에서 받은 오류정보를 객체로 저장함.
                setErrors(error.response.data);

            } else { //입력 값 이외에 발생하는 다른 오류와 관련됨. 
                setErrors((previous) => ({ ...previous, general: '회원가입 중에 오류가 발생하였습니다.' }));
            }
        }
    };
                ...
                <Col md={6}>
                    <Card>
                        <Card.Body>
                            ...
                            {/* 일반 오류 발생 시 사용자에게 alert 메시지를 보여줌. */}
                            {/* contextual message : 상황에 맞는 적절한 색상 */}
                            {errors.general && <Alert variant="danger">{errors.general}</Alert>}

                            {/* !!연산자 :  어떠한 값을 강제로 boolean 형태로 변환해주는 JS기법.
                                isInvalid 속성 : 해당 control의 유효성을 검사하는 속성.
                                값이 true면 Form.Control.Feedback에 빨간색으로 오류 메시지를 보여줌.
                            */}
